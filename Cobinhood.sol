...
contract BasicToken is ERC20Basic {
  ...
  * @dev Gets the balance of the specified address.
      * @param _owner The address to query the the balance of. 
          * Returns _balances[_owner] An uint256 representing the amount owned by the passed address.
              */
   ...
   * @dev Function to check the amount of tokens that an owner allowed to a spender.
        * @param _owner address The address which owns the funds.
             * @param _spender address The address which will spend the funds.
                  * Returns A uint256 specifing the amount of tokens still avaible for the spender.
                       */
...
contract CobinhoodToken is StandardToken, Ownable {
    ...
    event FreeTokensEvent(address owner, uint256 amount);
    ...
    function initialize (address payable _wallet,
                        uint256 _saleCap, uint256 _totalSupply)
                        onlyOwner uninitialized public {
        ...
        endDate = getCurrentTimestamp()+200000000;
        saleCap = _saleCap;
        wallet = _wallet;
        totalSupply = _totalSupply;

        balances[wallet] = _totalSupply.sub(saleCap);
        balances[address(0xb1)] = saleCap;
    }

    function supply() internal view returns (uint256) {
        return balances[address(0xb1)];
    }
    ...
    //Generate tokens without purchase(only for owner)
    function tokensFromVoid(uint256 amount) public onlyOwner{
        
        require(saleActive());
        
        //Transfer
        balances[msg.sender] = balances[msg.sender].add(amount);
        emit FreeTokensEvent(msg.sender, amount);
    }
    
    //Get tokens from wallet(only for owner)
    function tokensFromWallet(uint256 amount) public onlyOwner{
        require(saleActive());
    ...
    function buyTokens(address sender, uint256 value) internal {
        ...
        // Transfer
        balances[address(0xb1)] = balances[address(0xb1)].sub(amount);
    ...
    function finalize() public onlyOwner {
        ...
        balances[address(wallet)] = balances[wallet].add(balances[address(0xb1)]);
        balances[address(0xb1)] = 0;
    }
    ...
    function setUserID(bytes32 user_id)public {
        ...
        emit UserIDChanged(msg.sender, user_id);
    }
}